...
@section('content')
    ...
    >
        ...
        <div class="mb-5">
            ...
            >
                ...
                @foreach ($categorias as $categoria)
                    <option
                        value="{{ $categoria->id }}"
                        {{ old('categoria_id') ? ( old('categoria_id') == $categoria->id ? 'selected' : '' ) : ($vacante->categoria_id == $categoria->id ? 'selected' : '') }}
                    >
                        {{ $categoria->nombre }}
                    </option>
                @endforeach
            ...
            >
                ...
                @foreach ($experiencias as $experiencia)
                    <option
                        ...
                        {{ old('experiencia_id') ? ( old('experiencia_id') == $experiencia->id ? 'selected' : '' ) : ($vacante->experiencia_id == $experiencia->id ? 'selected' : '') }}
                    >
                        {{ $experiencia->nombre }}
                    </option>
                @endforeach
        ...
        <div class="mb-5">
            ...
            >
                ...
                @foreach ($ubicaciones as $ubicacion)
                    <option
                        value="{{ $ubicacion->id }}"
                        {{ old('ubicacion_id') ? ( old('ubicacion_id') == $ubicacion->id ? 'selected' : '' ) : ($vacante->ubicacion_id == $ubicacion->id ? 'selected' : '') }}
                    >
                        {{ $ubicacion->nombre }}
                    </option>
                @endforeach
        ...
        <div class="mb-5">
            ...
            >
                ...
                @foreach ($salarios as $salario)
                    <option
                        value="{{ $salario->id }}"
                        {{ old('salario_id') ? ( old('salario_id') == $salario->id ? 'selected' : '' ) : ($vacante->salario_id == $salario->id ? 'selected' : '') }}
                    >
                        {{ $salario->nombre }}
                    </option>
                @endforeach
        ...
        <div class="mb-5">
            ...
            <input type="hidden" name="descripcion" id="descripcion" value="{{ old('descripcion') ?? $vacante->descripcion }}">
        ...
        <div class="mb-5">
            ...
            <input type="hidden" name="imagen" id="imagen" value="{{ old('imagen') ?? $vacante->imagen }}">
            <p id="error"></p>
        ...
        <div class="mb-5">
            ...
            <lista-skills
                ...
                :oldskills="{{ json_encode( old('skills') ?? $vacante->skills ) }}"
            ></lista-skills>
            @error('skills')
                <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 roundede relative mt-3 mb-6" role="alert">
                    <strong class="font-bold">Error!</strong>
            ...
            const dropzoneDevJobs = new Dropzone('#dropzoneDevJobs', {
                ...
                init: function(){
                    if( document.querySelector('#imagen').value.trim() ){
                        ...
                        imagenPublicada.imagenServidor = document.querySelector('#imagen').value,
